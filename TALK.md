...
2026-09-13, carries both readings): the six figures under `report/figs/` (the learning curve, the
n_B plot, the model ladder, the reader chain, the scatter of thinking's effect against baseline
probe AUC that scene 6 ends on, and H5's forest plot of what the concept block adds on top of the
pixels), every table
