...
Three things here were learned from the service rather than designed:
  ...
  * **Thinking must be turned off explicitly.** With reasoning left on, the primary model spends
    ...
    `extra_body`.
  * **One served model files its answer under `reasoning_content` even so**, so a reply is read
    from `content` first and from `reasoning_content` only when `content` is empty.
...
class Reply:
    ...
    from_reasoning: bool           # the text came out of reasoning_content, not content
...
def read_reply(response, transport_attempts: int = 1) -> Reply:
    ...
    `content` first; `reasoning_content` only when `content` is empty, because one served model
    files its answer there even with thinking switched off. Which one it came from is recorded, so
    a later reader can tell a normal reply from that fallback rather than guess.
    """
