...
**Amended 2026-09-18, twice more.** The figures were held to the same plain-language standard as
the prose, and one name was fixed per thing: the textbook lists **visual features**, the model
returns **feature scores**, and what arms C, P and C+P each fit is a **classifier**. "Checklist"
is not used anywhere. The arm names live in `study.style.arms` and so the rename was made in the
export. Separately, the callouts were cut from ten to three — *No clinician has read this bank*,
...
is defined on. The diagram and every sentence that repeated the claim were corrected; since
2026-09-18 the drawing carries it instead of the words, three boxes each saying
"classifier". *A new section, "The models we asked"*, sits between the arms and how the
